/* cpu_DES.c
 * CPU implementation of DES encryption/decryption in ECB mode.
 */
...
// Apply permutation
uint64_t permute(uint64_t in, const uint8_t *table, int n)
{
    uint64_t out = 0;
    for (int i = 0; i < n; i++)
    {
...
// DES function: process a single 64-bit block
void des_encrypt_block(const uint64_t *in, uint64_t *out)
{
    uint64_t block = *in;
    // Initial Permutation
    ...
    {
        uint32_t tmp = R;
        R = L ^ feistel(R, g_subkeys[rnd]);
        L = tmp;
    }
    // Pre-output: swap L and R
    uint64_t preout = ((uint64_t)R << 32) | L;
    // Final Permutation
    *out = permute(preout, FP, 64);
}

// Generate subkeys
void generate_subkeys(uint64_t key, uint64_t *subkeys, int decrypt)
{
    // PC-1 permutation
    uint64_t perm = permute(key, PC1, 56);
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        temp_subkeys[i] = permute(CD << 8, PC2, 48);
    }
...
int main(int argc, char *argv[])
{
    ...
    generate_subkeys(key, g_subkeys, decrypt);

    // Process blocks sequentially
    printf("%s %zu blocks...\n", decrypt ? "Decrypting" : "Encrypting", num_blocks);
    for (size_t i = 0; i < num_blocks; i++)
    {
        des_encrypt_block(&h_in[i], &h_out[i]);
    }

    end_time = clock();
    double elapsed_time = ((double)(end_time - start_time) / CLOCKS_PER_SEC) * 1000.0; // Convert to milliseconds
    printf("Time taken: %0.2f milliseconds\n", elapsed_time);
